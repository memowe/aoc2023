...
instance Show Card where
  show = return . ("J23456789TQKA" !!) . fromEnum
...
typeOfHand h = case length <$> handGroups of
  (5:_)     -> FiveOfAKind
  (4:_)     -> FourOfAKind
  (3:2:_)   -> FullHouse
  (3:_)     -> ThreeOfAKind
  ...
  (2:_)     -> OnePair
  _         -> HighCard
  where (js, njs)   = partition (== CJ) $ handToList h
        handGroups  = case grpLenSort $ group (sort njs) of
                        (g:gs)  -> (js ++ g) : gs
                        []      -> [js]
        grpLenSort  = sortBy (comparing $ Down . length)
